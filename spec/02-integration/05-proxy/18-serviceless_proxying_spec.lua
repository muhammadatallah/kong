  ...
  describe("Serviceless Proxying [#" .. strategy .. "]", function()
    describe("[http]", function()
      ...
      it("proxies http to http (request-line)", function()
        local request = assert(http_request.new_from_uri(HTTP_UPSTREAM_URI))
      ...
      pending("proxies http to https (connect)", function()
        ...
        local request = assert(http_request.new_from_uri(HTTPS_UPSTREAM_URI))
        request.proxy = HTTP_PROXY_URI
        request.version = 1.1
        request.tls = true
        request.ctx = ctx
    ...
    describe("[stream]", function()
      ...
      lazy_setup(function()
        ...
        assert(helpers.start_kong({
          ...
          origins       = "tcp://127.0.0.1:19000=" ..
                          "tcp://" .. HTTP_PROXY_HOST ..  ":" .. STREAM_UPSTREAM_PORT .. "," ..
                          "tls://127.0.0.1:19443=" ..
                          "tls://" .. HTTP_PROXY_HOST ..  ":" .. STREAM_UPSTREAM_SSL_PORT
        }))
      end)
      ...
      it("proxies tls to tls (origins)", function()
        local tcp = ngx.socket.tcp()
        assert(tcp:connect(HTTP_PROXY_HOST, 19443))

        -- TODO: should SNI really be mandatory?
        assert(tcp:sslhandshake(false, "this-is-needed.org"))
